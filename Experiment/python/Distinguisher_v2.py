...
from Target_samples import *
from Dual_short_vectors import *
...
for target_LWE  in target_list_LWE :
	...
	for w in short_vector_list : 
		ww , tt = np.array(w) , np.array(target_LWE)
		D_LWE  += np.cos(2/modulus * np.pi * np.dot(ww,tt))
	data_LWE.append((1/W_len) * D_LWE)

print(np.mean(data_LWE))

data_unif=[]
for target_unif in target_list_unif :
	D_unif = 0
	for w in short_vector_list : 
		ww , tt = np.array(w) , np.array(target_unif)
		D_unif += np.cos(2/modulus * np.pi * np.dot(ww,tt))		
	data_unif.append((1/W_len) * D_unif)

print(np.mean(data_unif))
...
for target_LWE  in target_list_LWE :
	...
	for w in short_vector_list : 
		...
		D_LWE  += np.cos(2/modulus * np.pi * RR(ww.inner_product(tt)))
...
for target_unif in target_list_unif :
	...
	for w in short_vector_list : 
		...
		D_unif += np.cos(2/modulus * np.pi * RR(ww.inner_product(tt)))		
