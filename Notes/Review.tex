...
% active/désactive les commentaires facilement
\newif\ifshowcomments
\showcommentstrue   % mettre \showcommentsfalse pour tout masquer
...
Suppose now we are able to sample short vectors $w_1,\ldots , w_N$ in $\LL_q(A_1)^\perp$ according to $\D = \D_{\LL_q(A_1)^\perp , s^{-1}}$, and that $N$ is big enough that $g_W$ and $f_{\D}$ \TN{Pas introduit encore ?} are indistinguishably close. Then 
\[g_W(t) \approx \rho_s(\LL_q(A_1) +t ) / \rho_s(\LL_q(A_1) ).\]

We suppose that $s$ is chosen so that 
\[\argmin_{s\in \ZZ^n}g_W(s) \approx \argmin_{s\in \ZZ^n} f_{\D}(t - A_0 s) = s_0.\] 
Then, as $g_W$ (as is $f_\D$) is $\LL_q(A_1)$-periodic and that $q\ZZ^n\subset \LL_q(A_1)$, $g_W$ can be seen as a function defined on the finite group $\ZZ_q^n$. One can use the Fast Fourier Transform algorithm to evaluate all values of $g_W$ and thus recover $s_0$. \TN{Pour être précis, tu calcules toutes les valeurs de $g_W$ pour ensuite prendre l'argmin ?}
...
The following \TN{Référencer plus clairement le lemme, car il ne vient pas juste après.} lemma ensures that $g_W$ satisfies a gaussian-type concentration inequality around $f_\D$.
